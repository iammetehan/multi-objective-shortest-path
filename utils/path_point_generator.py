...
def generate_path_points(path_points, obstacles):
    ...
    for i in range(number_of_path_points):
        path_points.append(generate_path_point(obstacles, path_points))

    path_points.append((goal_x, goal_y))


def generate_path_point(obstacles, path_points):
    axes = parser['Plot Axes']
    ...
    while True:
        ...
        if not path_point_near_obstacle(x, y, obstacles) and not path_point_near_another(x, y, path_points):
            return (x, y)


def path_point_near_obstacle(x, y, obstacles):
    point = Point(x, y)
...
def path_point_near_another(x, y, path_points):
    point1 = Point(x, y)
